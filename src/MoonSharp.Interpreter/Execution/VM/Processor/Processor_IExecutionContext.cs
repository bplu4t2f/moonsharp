...
namespace MoonSharp.Interpreter.Execution.VM
{
	sealed partial class Processor
	{
		...
		internal DynValue GetBinaryMetamethod(DynValue op1, DynValue op2, string eventName)
		{
			...
			{
				DynValue meta = op1.UserData.GetDescriptor(this.m_Script.TypeRegistry).MetaIndex(this.m_Script,
			...
			{
				DynValue meta = op2.UserData.GetDescriptor(this.m_Script.TypeRegistry).MetaIndex(this.m_Script,
		...
		internal DynValue GetMetamethod(DynValue value, string metamethod)
		{
			...
			{
				DynValue v = value.UserData.GetDescriptor(this.m_Script.TypeRegistry).MetaIndex(m_Script, value.UserData.Object, metamethod);
				if (v != null)
					return v;
			}
