		...
		private void FillMemberList(UserDataRegistry registry)
		{
			...
			if (!type.IsDelegateType())
			{
				...
				foreach (ConstructorInfo ci in Framework.Do.GetConstructors(type))
				{
					...
					AddMember("__new", MethodMemberDescriptor.TryCreateIfVisible(registry, ci, this.AccessMode, false));
				}
			...
			foreach (MethodInfo mi in Framework.Do.GetMethods(type))
			{
				...
				MethodMemberDescriptor md = MethodMemberDescriptor.TryCreateIfVisible(registry, mi, this.AccessMode, false);
